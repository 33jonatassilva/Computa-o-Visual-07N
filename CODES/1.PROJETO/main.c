...
//------------------------------------------------------------------------------
// Exemplo: 04-invert_image (com equalização de histograma)
//------------------------------------------------------------------------------
...
static void render(void)
{
    ...
    draw_button(h_window.renderer, &h_button, "Equalizar Histograma");
...
float calculate_average_intensity()
{
    ...
    for (int i = 0; i < pixelCount; i++) {
        Uint8 r, g, b, a;
        SDL_GetRGBA(pixels[i], format, NULL, &r, &g, &b, &a);
        soma += r; // A imagem é tons de cinza
    }
...
void equalize_histogram()
{
    ...
    for (int i = 0; i < 256; ++i) {
        sum += histogram[i];
        histogram_equalized[i] = roundf(((float)sum / (float)pixelCount) * 255.0f);
    }
}
...
const char *classify_deviation_string(float deviation)
{
    ...
    if (deviation <= 104.0f) return "Contraste medio";
...
int main(int argc, char *argv[])
{
    ...
    h_button.rect = (SDL_FRect){ .x = 175, .y = DEFAULT_H_WINDOW_HEIGHT - 100, .w = 190, .h = 35 };
